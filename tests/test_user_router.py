...
async def auth_token(client):
    import random
    telegram_id = random.randint(10000, 99999)  # уникальный каждый раз

    data = {'telegram_id': telegram_id, 'username': 'Ivan', 'role': 'Преподаватель', 'is_banned': False}
...
async def test_change_name(new_name, auth_token, client):
    response = await client.put('/users/me/name',
                                json={'name': new_name},
                                headers={'Authorization': f"Bearer {auth_token}"})
    assert response.status_code == 200
    data = response.json()
...
async def test_soft_delete(client, telegram_id, username, role, is_banned, auth_token):
    response = await client.post('/users/create', json= {'telegram_id': telegram_id, 'username': username, 'role': role, 'is_banned': is_banned})

    token_response = await client.post('/auth/login', json = {'telegram_id': telegram_id})
    token = token_response.json()['access_token']

    response = await client.delete(f'/users/soft_del', headers= {'Authorization': f"Bearer {token}"})
...
async def test_restore_user(client, telegram_id, username, role, is_banned):
    response_create = await client.post('/users/create', json={'telegram_id': telegram_id, 'username': username, 'role': role,
                                                        'is_banned': is_banned})
    token_response = await client.post('/auth/login', json={'telegram_id': telegram_id})
    token = token_response.json()['access_token']
    response_delete = await client.delete('/users/soft_del', headers={'Authorization': f"Bearer {token}"})
    response_restore = await client.put('/users/restore_user', headers={'Authorization': f"Bearer {token}"})

    print(response_restore.status_code)
    print(response_restore.json())

...
async def created_answer(client):
    response_create = await client.post('/users/create',
                                        json={'telegram_id': 102, 'username': 'OLEG', 'role': 'Админ',
                                              'is_banned': False})
    token_response = await client.post('/auth/login', json={'telegram_id': response_create.json().get('telegram_id', 101)})
    token = token_response.json()['access_token']

    actual_user_id = response_create.json().get('id', 1)
...
async def test_get_answer_by_id(client, created_answer):
    ...
    response = await client.get(f'/answers/{answer.get('id', 1)}',
                                headers= {'Authorization': f"Bearer {token}"} )
...
async def created_answer_all(client):
    response_create = await client.post('/users/create',
                                        json={'telegram_id': 103, 'username': 'ANNA', 'role': 'Админ',
                                              'is_banned': False})
    token_response = await client.post('/auth/login', json={'telegram_id': response_create.json().get('telegram_id', 103)})
    token = token_response.json()['access_token']
...
async def test_get_all_answer(client, created_answer_all):
    answer, token = created_answer_all
    response = await client.get(f'/answers',
                                headers={'Authorization': f"Bearer {token}"})
    assert response.status_code == 200